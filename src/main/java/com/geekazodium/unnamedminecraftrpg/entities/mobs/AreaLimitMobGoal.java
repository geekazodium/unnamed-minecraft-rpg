...
public class AreaLimitMobGoal implements Goal<Mob>, Listener{
    private GoalKey<Mob> key = GoalKey.of(Mob.class,new NamespacedKey(Main.getInstance(),"pathfindback"));
    ...
    public AreaLimitMobGoal(Mob mob,Location location, double distance){
    ...
    private Location getTargetLocation(){
        return location.clone().toCenterLocation().add(
                nextIntRandomSign(distance),0,nextIntRandomSign(distance)
        );
    }

    private int nextIntRandomSign(int bound){
